...
class YouTubeFlowPlugin extends Plugin {
   ...
   async onload() {
      ...
      this.app.workspace.onLayoutReady(async () => {
         ...
         // Vérifier TOUTES les leaves, pas seulement markdown
         const allLeaves = this.app.workspace.getLeavesOfType('markdown')
            .concat(this.app.workspace.getLeavesOfType('empty'));
         
         // Nettoyer les vues vides existantes
         allLeaves.forEach(leaf => {
            if (leaf.view?.getViewType() === 'empty') {
               console.log("Nettoyage d'une vue vide:", leaf.id);
               leaf.detach();
            }
         });
         ...
         // Revérifier les leaves après nettoyage
         const remainingLeaves = this.app.workspace.getLeavesOfType('markdown')
            .concat(this.app.workspace.getLeavesOfType('empty'));
         
         console.log("Leaves après nettoyage:", remainingLeaves.map(leaf => ({
            id: leaf.id,
            type: leaf.view?.getViewType(),
            hasIframe: !!leaf.view?.containerEl?.querySelector('iframe'),
            isYouTube: leaf.view?.containerEl?.querySelector('iframe[src*="youtube"]') !== null
         })));

         // Première vérification par ID
         const existingYouTubeLeafById = this.settingsManager.settings.youtubeLeafId ? 
            this.app.workspace.getLeavesOfType('markdown')
               .find(leaf => leaf.id === this.settingsManager.settings.youtubeLeafId) : null;

         console.log("Vérification leaf YouTube:", {
            savedId: this.settingsManager.settings.youtubeLeafId,
            found: !!existingYouTubeLeafById,
            leafId: existingYouTubeLeafById?.id
         });

         // Vérification plus stricte
         const existingYouTubeLeafByIframe = remainingLeaves.find(leaf => 
            leaf.view?.containerEl?.querySelector('iframe[src*="youtube"]') !== null
         );

         if (existingYouTubeLeafByIframe) {
            console.log("Vue YouTube trouvée !", {
               id: existingYouTubeLeafByIframe.id,
               type: existingYouTubeLeafByIframe.view?.getViewType()
            });
            this.settingsManager.settings.youtubeLeafId = existingYouTubeLeafByIframe.id;
            await this.settingsManager.save();
         }

         console.log("Résultat final:", {
            youtubeLeafId: this.settingsManager.settings.youtubeLeafId,
            existingLeaf: !!existingYouTubeLeafByIframe,
            leafDetails: existingYouTubeLeafByIframe ? {
               id: existingYouTubeLeafByIframe.id,
               type: existingYouTubeLeafByIframe.view?.getViewType()
            } : null
         });
...
class SplitView {
   ...
   async open() {
      ...
      this.settingsManager.settings.youtubeLeafId = leaf.id;
      await this.settingsManager.save();
      ...
      const handleClose = async () => {
         ...
         this.settingsManager.settings.youtubeLeafId = null;  // Réinitialiser l'ID
         await this.settingsManager.save();
         SplitView.activeLeaf = null;
         SplitView.activeView = null;
      ...
      if (displayMode === 'tab') {
         ...
         leaf.on('detach', async () => {
            console.log("Fermeture de la leaf YouTube:", this.settingsManager.settings.youtubeLeafId);
            this.settingsManager.settings.youtubeLeafId = null;
            await this.settingsManager.save();
   ...
   async close() {
      if (SplitView.activeLeaf) {
         SplitView.isClosing = true;
         await SplitView.activeLeaf.detach();
         SplitView.isClosing = false;
      }
   }
}
