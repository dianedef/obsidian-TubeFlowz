...
class YouTubeFlowPlugin extends Plugin {
   ...
   async onload() {
      ...
      this.app.workspace.onLayoutReady(async () => {
         ...
         // Debug détaillé de chaque leaf
         allLeaves.forEach(leaf => {
            console.log("Analyse leaf:", {
               id: leaf.id,
               viewType: leaf.view?.getViewType(),
               mode: leaf.view instanceof MarkdownView ? leaf.view.getState().mode : 'unknown',
               savedId: this.settingsManager.settings.youtubeLeafId,
               isMatch: leaf.id === this.settingsManager.settings.youtubeLeafId,
               hasIframe: !!leaf.view?.containerEl?.querySelector('iframe')
            });
         });

         // Vérification plus stricte
         const existingYouTubeLeaf = allLeaves.find(leaf => {
            const isCorrectId = leaf.id === this.settingsManager.settings.youtubeLeafId;
            const hasYouTubeMode = leaf.view instanceof MarkdownView && 
                                 leaf.view.getState().mode === 'youtube-player';
            const hasIframe = leaf.view?.containerEl?.querySelector('iframe') !== null;
            ...
            return isCorrectId || (hasYouTubeMode && hasIframe);
         });

         console.log("Résultat final:", {
            youtubeLeafId: this.settingsManager.settings.youtubeLeafId,
            existingLeaf: !!existingYouTubeLeaf,
            leafDetails: existingYouTubeLeaf ? {
               id: existingYouTubeLeaf.id,
               mode: existingYouTubeLeaf.view instanceof MarkdownView ? 
                     existingYouTubeLeaf.view.getState().mode : 'unknown'
            } : null
         });
